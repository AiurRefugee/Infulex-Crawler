...
const listenPOSTCreateTask = (app) => {
    ...
    app.post('/createTask', async (req, res) => {
        const { mediaId, mediaType, backdropPath, title } = req.body
        console.log('createTask', mediaId, mediaType, backdropPath, title)
        if (!mediaId || !mediaType || !backdropPath || !title) {
            res.json({
                code: 400,
                message: '参数错误'
            })
        }
        const insertRes = await createTask(mediaId, mediaType, backdropPath, title)
        if (insertRes) {
            res.json({
                code: 200,
                message: 'createTask 创建成功'
            })
        } else {
            res.json({
                code: 500,
                message: 'createTask 创建失败'
            })
        }
    })
}
...
module.exports = listenPOSTCreateTask