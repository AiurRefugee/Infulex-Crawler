...
const toogleStyle = () => {
  ...
  showTitle.value = scrollArea.value.scrollTop > 100
}
...
<template>
  <!-- 顶部占位 -->
  <div class="bg-white trans center" :style="{ height: focus ? '0' : '30px'}">
    <text class="trans self-end" :style="{
      opacity: showTitle ? '1' : '0'
    }">搜索</text>
  </div>
  <div 
    class="w-full h-full px-4 overflow-auto"  
    ref="scrollArea"
    @scroll="toogleStyle"> 
    ...
    <div class="inputRow w-full h-[48px] py-[8px] flex sticky top-0 bg-white">
      <div class="inputArea w-full px-2 flex items-center bg-gray-300 rounded-lg ">
        ...
        <input v-model="searchText" class="w-full px-2 text-[1.2em] text-white" placeholder="电影，剧集"
          @focus="focus = true" />
      </div>
      <text :style="{
        width: focus ? '50px' : '0'
    ...
    <div class="w-full h-full" v-if="!focus">
      ...
      <div class="buttonGrid w-full h-full">
        <div class="browseClassButton center h-[15vh]" :style="{ background: genre.background }" v-for="genre in genres"
          :key="genre.id">
