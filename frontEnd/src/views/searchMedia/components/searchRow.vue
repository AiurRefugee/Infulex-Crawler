...
<template>
    <div 
        :class="[
            ...
            focused ? 'bg-light-700 py-2' : 'bg-light-900', 
        ]" 
        class="flex sticky top-0 pb-2 z-[100]"  
    >
        <div  
            class="inputRow w-full pl-4 flex transSlow"
        >
            <div class="inputArea bg-light-500 w-full px-2 flex items-center rounded-lg">
                <img class="searchIcon h-[15px] fill-slate-200" src="@/assets/icons/search.svg" />
                <input v-model="searchText" class="w-full px-2 h-[1.6em] text-[1.2em] text-white" placeholder="电影，剧集"
                    @focus="handleFocus" @input="search" @keydown.enter="search" />
            </div>
            <text :style="{
                width: focused ? '50px' : '0',
            }" class="center mx-2 whitespace-nowrap text-red-600 overflow-hidden trans cursor-pointer"
                @click="handleBlur">取消</text>
        </div>
    </div>
</template>
<style>
