...
const inTaskListTitle = computed(() => {
  if (inTaskList.value) {
    console.log(inTaskList.value);
    const { msgs } = inTaskList.value
    const len = msgs.length
    return msgs[len - 1].data.time
  } else {
    return "添加任务";
  }
});
...
<template>
  <div class="px-4">
    ...
    <div class="h-full flex gap-3 flex-shrink-0">
      ...
      >
        <svg 
          :class="isFavorite == null ? 'opacity-60 cursor-not-allowed' : ''"
          class="icon w-1/2" 
