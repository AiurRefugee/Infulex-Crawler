...
<template>
    <div class="loadImg center w-full h-full rounded-lg">
        <img 
            :src="src" alt="image"
            :class="loading ? 'hidden' : ''"  
            class="imageArea w-full h-full object-cover rounded-lg"
            @load="loading = false"
        />
        <svg viewBox="0 0 1024 1024" class="w-[60px] aspect-square" v-if="loading">
            <path
